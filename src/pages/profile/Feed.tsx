...
import { usePosts } from "../main/_components/postProvider";
...
export default function ForYouFeed(profile: Profile) {
  const { id } = useParams();
  ...
  const fetchPosts = async (pageParam = 0) => {
    const isOtherUser = profile.profile?.id || (id && id.includes("@"));
    const userId = isOtherUser ? profile.profile?.id : user.id;
    const userEmail = isOtherUser ? profile.profile?.email : user.email;
  ...
  useEffect(() => {
    if ((profile.profile?.id || (id && id.includes("@"))) || (!profile.profile && user && user.email)) {
      fetchPosts(page);
    }
  }, [page, refetch, profile, user]);
